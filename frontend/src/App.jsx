...
import { Terminal, Github, Loader2, ExternalLink, CheckCircle2, AlertCircle, RefreshCw } from 'lucide-react';
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const fetchLogs = async () => {
      try {
        const { data } = await axios.get(`http://localhost:9000/logs/${deploymentId}`);
  ...
  const handleDeploy = async (e) => {
    ...
    try {
      ...
      setStatus('building');
      setDeployedUrl(`http://${project.subDomain}.localhost:8000`);
  ...
  useEffect(() => {
    const handleMessage = (message) => {
      ...
      if (message.includes('Authentication failed') || message.includes('could not read Username')) {
          setStatus('error');
          setErrorDetails("Deployment failed. If this is a private repository, please make it Public in GitHub settings and click Redeploy.");
      }
    };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 flex flex-col items-center justify-center p-4 font-sans selection:bg-blue-500/30">
      ...
      <div className="mb-10 text-center space-y-2">
        ...
        <p className="text-slate-400 text-lg">Deploy from GitHub to Edge in seconds.</p>
      </div>
      ...
      <div className="w-full max-w-3xl bg-slate-900/50 backdrop-blur-md border border-slate-800 rounded-xl shadow-2xl overflow-hidden ring-1 ring-white/10">
        ...
        {status !== 'idle' && (
          <div className="p-8">
            ...
            <div 
              ref={logContainerRef}
              className={`bg-[#0c0c0c] rounded-lg p-4 h-80 overflow-y-auto font-mono text-xs md:text-sm shadow-inner custom-scrollbar relative border ${status === 'error' ? 'border-red-900/30' : 'border-slate-800/50'}`}
            >
              {logs.length === 0 ? (
                ...
                <div className="space-y-1">
                  {logs.map((log, index) => (
                    <div key={index} className="break-all hover:bg-white/5 py-0.5 px-2 -mx-2 rounded transition-colors flex">
                      <span className="text-slate-600 select-none mr-3 shrink-0">{`>`}</span>
                      <span className={log.toLowerCase().includes('error') ? 'text-red-400' : 'text-slate-300'}>
